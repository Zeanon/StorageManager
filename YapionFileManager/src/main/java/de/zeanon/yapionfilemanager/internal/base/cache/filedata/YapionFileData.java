package de.zeanon.yapionfilemanager.internal.base.cache.filedata;
...
@SuppressWarnings({"rawtypes", "DefaultAnnotationParam"})
public class YapionFileData<M extends Map, E extends Map.Entry, L extends List> implements FileData<M, E, L>, Comparable<YapionFileData>, Serializable {
	 ...
	 */
	private transient @NotNull YAPIONObject yapionObject;
	 ...
	 */
	@Contract(pure = true)
	protected YapionFileData(final @NotNull CollectionsProvider<M, L> collectionsProvider) {
		this.collectionsProvider = collectionsProvider;
		this.yapionObject = this.collectionsProvider().newMap(); //NOSONAR
	}
	 ...
	 */
	@Contract(pure = true)
	protected YapionFileData(final @NotNull CollectionsProvider<M, L> collectionsProvider, final @NotNull M yapionObject) {
		this.collectionsProvider = collectionsProvider;
	...
	@Override
	public @NotNull M dataMap() {
		//noinspection unchecked
		return (M) this.yapionObject.getBackedMap();
	}
	...
	public @NotNull List<E> entryList() {
		//noinspection unchecked
		return this.internalEntryList(this.yapionObject);
	}
	...
	public @NotNull List<E> blockEntryList() {
		//noinspection unchecked
		return this.collectionsProvider.newList(new Class[]{Set.class}, this.yapionObject.entrySet());
	}
	...
	public boolean containsKey(final @NotNull String key) {
		final @NotNull String[] parts = key.split("\\.");
		//noinspection unchecked
		return this.internalContainsKey(this.yapionObject, parts, 0);
	}
	...
	public boolean containsKeyUseArray(final @NotNull String... key) {
		//noinspection unchecked
		return this.internalContainsKey(this.yapionObject, key, 0);
	}
	...
	public @Nullable Object get(final @NotNull String key) {
		final @NotNull String[] parts = key.split("\\.");
		return this.internalGet(this.yapionObject, parts);
	}
	...
	public @Nullable Object getUseArray(final @NotNull String... key) {
		return this.internalGet(this.yapionObject, key);
	}
	...
	public int size() {
		//noinspection unchecked
		return this.internalSize(this.yapionObject);
	}
	...
	public boolean isEmpty() {
		return this.yapionObject.isEmpty();
	}
	...
	private void initialInsert(final @Nullable Object value,
		...
		if (value == null) {
			...
			final Object tempValue = this.yapionObject.get(key[0]);
			//noinspection unchecked
			final @NotNull Map<String, Object> childMap =
					this.yapionObject.containsKey(key[0])
					&& tempValue instanceof Map
					? (Map) tempValue
					: this.collectionsProvider().newMap();
			//noinspection unchecked
			this.yapionObject.put(key[0], this.internalInsert(childMap, key, value, 1));
		}
	}
	...
	private void initialRemove(final @NotNull String... key) {
		if (key.length == 1) {
			this.yapionObject.remove(key[0]);
		} else {
			final Object tempValue = this.yapionObject.get(key[0]);
			if (tempValue instanceof Map) {
				//noinspection unchecked
				this.yapionObject.put(key[0], this.internalRemove((Map) tempValue, key, 1));
				if (((Map) this.yapionObject.get(key[0])).isEmpty()) {
					this.yapionObject.remove(key[0]);
				}
			}
		}
	...
	private @NotNull List<Map.Entry<String, Object>> internalEntryList(final @NotNull Map<String, Object> map) {
		...
		for (final @NotNull Map.Entry<String, Object> entry : map.entrySet()) {
			if (entry.getValue() instanceof DataMap) {
				//noinspection unchecked
				tempList.add(new YapionFileData.Node<>(entry.getKey(), this.internalEntryList((DataMap) entry.getValue())));
			}
		}
		return tempList;
	}
	 ...
	 */
	@Override
	public int compareTo(final @NotNull YapionFileData fileData) {
		return Integer.compare(this.yapionObject.size(), fileData.yapionObject.size());
	}
	...
	public @NotNull String toString() {
		return this.yapionObject.toString();
	}
