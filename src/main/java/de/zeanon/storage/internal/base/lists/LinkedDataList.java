...
@SuppressWarnings({"SuspiciousMethodCalls", "SuspiciousToArrayCall", "unused"})
@NoArgsConstructor
@AllArgsConstructor
public class LinkedDataList<E> implements DataList<E> {

	@NotNull
	private List<E> localList = new LinkedList<>();

	public LinkedDataList(final @NotNull DataList<E> list) {
		this.localList = new LinkedList<>(list.getList());
	}
	...
	public DataList<E> subList(int fromIndex, int toIndex) {
		return new LinkedDataList<>(this.localList.subList(fromIndex, toIndex));
	}
